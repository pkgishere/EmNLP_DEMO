...
headers=\
{
    'Authorization':'Bearer 239e42e672184f0190ba27c708e810af',
...
def getIntentList():
    try:
        ...
        print("THESIS SIMILARITY GET INTENT LIST: [Errno {0}]".format(e))
...
def createIntent(name):
    ...
    try:
        ...
        print("THESIS SIMILARITY DF CREATED INTENT:",name)
    except Exception as e:
        print("THESIS SIMILARITY DF CREATE INTENT:[Errno {0}]".format(e))

def deleteIntent(id):
    try:
        url="https://api.dialogflow.com/v1/intents/"+str(id) +"?v=20150910"
        conn = requests.delete(url,data = {}, headers = headers)
        print("THESIS SIMILARITY DF DELETED INTENT:",conn.status_code,conn.reason)
    except Exception as e:
        print("THESIS SIMILARITY DF DELETED INTENT:[Errno {0}]".format(e))
...
def resetApplication():
    ...
    for key in IntentList:
        ...
        if (key == "Default Welcome Intent"):
            continue;
        print("THESIS SIMILARITY RESET IS DELETING INTENT:",key)
        deleteIntent(IntentList[key])
    print("THESIS SIMILARITY RESET DONE")
...
def add_utterance(input,intent):
    ...
    try:
        ...
        print ("THESIS SIMILARITY ADDED DATA IN INTENT " + intent)
    except Exception as e:
        print("THESIS SIMILARITY DF [Errno {0}]".format(e))
...
def getIntent(id):
    try:
        ...
        print("THESIS SIMILARITY [Errno {0}]".format(e))
